...
; PASMO PORT: TASM's #define remapping (DEFB->.BYTE etc.) removed -- pasmo
; understands DEFB/DEFW/DEFM/ORG/EQU natively, no remapping needed.
;#define DEFB .BYTE      
;#define DEFW .WORD
;#define DEFM .TEXT
;#define ORG  .ORG
...
;; CL-SET
L0DD9:  LD      HL,SAFE_ZERO_BYTE
                                ; --- PATCHED (see Decisions.md): was
                                ; "LD HL,$5B00". Harmless for the screen
                                ; path below (CL-ADDR overwrites HL
                                ; completely), and the printer path's
                                ; own column-offset addition further
                                ; down is neutralized separately (see
                                ; the JP retarget below) -- this initial
                                ; value only matters for the printer
                                ; case, and it's exactly the value we
                                ; want it forced back to anyway.
        BIT     1,(IY+$01)      ; test FLAGS  - is printer in use ?
...
L0DF4:  LD      A,$21           ; the column $01-$21 is reversed
        ...
        ADD     HL,DE           ; and add to base address
                                ; (for the printer case, this offsets
                                ; away from SAFE_ZERO_BYTE by 0-32 --
                                ; corrected back below, screen case is
                                ; untouched by that correction)

        JP      CL_PRINTER_FIX  ; --- PATCHED (see Decisions.md): was
                                ; "JP L0ADC" (PO-STORE) directly. Same
                                ; 3-byte JP shape, so nothing here
                                ; shifts. The stub forces HL back to
                                ; SAFE_ZERO_BYTE for the printer case
                                ; only (the column offset above would
                                ; otherwise walk HL across a 33-byte
                                ; range), then rejoins PO-STORE exactly
                                ; as before -- the screen case passes
                                ; through unchanged.
; ----------------
; Handle scrolling
; ----------------
...
L0ECD:  DI                      ; disable interrupts
        LD      HL,SAFE_ZERO_BYTE
                                ; --- PATCHED (see Decisions.md): was
                                ; "LD HL,$5B00". COPY-LINE's own read
                                ; loop (COPY-L-3) is separately patched
                                ; below to never advance past this byte.
...
;; CLEAR-PRB
L0EDF:  LD      HL,SAFE_ZERO_BYTE
                                ; --- PATCHED (see Decisions.md): was
                                ; "LD HL,$5B00". This routine only ever
                                ; writes (256 bytes, never reads back),
                                ; and real Z80 writes to ROM addresses
                                ; are already a no-op regardless of
                                ; which one -- so unlike CL-SET/
                                ; COPY-LINE, no loop rewrite is needed
                                ; here for correctness. Redirected
                                ; anyway for consistency: every site
                                ; that used to reference the real
                                ; printer buffer now starts from the
                                ; same single documented address.
...
L0F14:  LD      E,(HL)          ; fetch a byte from line.
        NOP                     ; --- PATCHED (see Decisions.md): was
                                ; "INC HL". HL now stays fixed at
                                ; SAFE_ZERO_BYTE for all 256 iterations
                                ; of this loop (8 lines x 32 bytes)
                                ; instead of walking through a range --
                                ; every read returns the same
                                ; guaranteed-zero byte.
...
;   intact.
        ...
        CALL    RND_BOOT_STUB   ; --- PATCHED (see Decisions.md): was
                                ; "CALL L0EDF" (CLEAR-PRB, now neutered
                                ; -- see Patch 7). Same 3-byte CALL
                                ; shape. The stub preserves the original
                                ; call exactly, then also seeds the RND
                                ; table -- this is the one-time NEW/
                                ; cold-boot system-initialization
                                ; sequence, the natural place to do it
                                ; once rather than on every RND call or
                                ; accepting a slow warm-up transient.
                                ; The buffer is clear.
...
;; LINE-NEW
L1B9E:  CALL    CACHE_LOOKUP_OR_REAL ; was: CALL L196E (LINE-ADDR) directly --
                                 ; now checks the 32-slot cache first,
                                 ; falling through to the real, unmodified
                                 ; LINE-ADDR on a miss. Same 3-byte CALL
                                 ; either way -- same contract (HL=address,
                                 ; Z=exact match) -- nothing downstream
                                 ; needs to change.
...
;; RAND-1
L1E5A:  JP      RND_RESEED      ; --- PATCHED (see Decisions.md): was
                                ; "LD ($5C76),BC; RET". Same 3-byte JP
                                ; shape leaves 2 bytes of the original
                                ; 5-byte slot as padding. RND_RESEED
                                ; itself ends with RET, so this is a
                                ; tail call -- returns to RANDOMIZE's
                                ; own caller exactly as before. The old
                                ; SEED system variable write is dropped
                                ; -- SEED is no longer read by anything
                                ; (S-RND uses the CMWC table instead).
        DEFB    $00,$00
...
; --- PATCHED (see Decisions.md): trampoline to NEW_CIRCLE_CORE, a
; Midpoint Circle Algorithm (Bresenham's circle) replacement for the
; original's rotation-formula/chord-polygon approach. Reached as fresh
; Z80 code (the immediately preceding instruction is "end-calc", so the
; calculator context is already closed by this point) -- a plain JP
; trampoline suffices, no end-calc-first trick needed.
;
; This replaces ONLY CIRCLE's own unique code (confirmed via an
; exhaustive check of the original's structure): CD-PRMS1, DRAW-LINE,
; and all other infrastructure shared with the DRAW command are
; completely untouched, since NEW_CIRCLE_CORE never calls into any of
; it -- it does its own direct pixel plotting via PLOT-SUB instead.
        ORG     $2331           ; original insertion point -- verified
                                 ; against baseline.bin before patching
        JP      NEW_CIRCLE_CORE

; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 8): NEW_TAN_CORE
; (67 bytes) relocated here, into CIRCLE's own spare trampoline padding,
; rather than sitting in the main free block -- the same "different
; routine hosted in a slot's own slack" pattern Entry 5 already used
; for NEW_COS_CORE in C-ENT's padding. This slot's own required header
; (JP NEW_CIRCLE_CORE, 3 bytes) is unrelated to and unaffected by this
; -- NEW_TAN_CORE is reached only via a separate JP from TAN's own
; trampoline at $37DA, never fallen into from here, so storing it in
; the bytes following CIRCLE's own JP is safe. 11 bytes of slack remain
; in the 81-byte slot (3 header + 67 core + 11 slack) -- the tightest
; fit of any cross-slot relocation done so far. This reclaims the 67
; bytes NEW_TAN_CORE previously occupied in the main free block.
;
; NEW_TAN_CORE -- independent Pade [3/2] rational approximant for tan.
; Reached only via the trampoline left at L37DA (table dispatch only --
; tan is never called from elsewhere in this ROM). Entry: angle in
; radians on top of the FP calculator stack. Exit: tan(angle) on top of
; the FP calculator stack, RET (or the same division-by-zero error the
; original raised for arguments at/near an asymptote, since the final
; division still goes through the ROM's own "division" calculator
; literal, unchanged).
;
; tan(x) ~= x(15-x^2)/(15-6x^2) is the Pade [3/2] approximant of tan
; around 0, accurate (~2e-4 max error) up to about x=pi/4. For angle
; magnitudes between pi/4 and pi/2, the complementary-angle identity
; tan(x) = sgn(x)/tan(pi/2-|x|) is used instead. See Alpha Decisions.md
; for the full get-argt-reuse derivation and verification.
;
; Uses calculator memories 0 (Y_tan), 1 (denom, transient), and 2
; (is_recip flag).
NEW_TAN_CORE:
        RST     28H             ;; FP-CALC              angle.
        DEFB    $39             ;;get-argt              Y_sin.  (mem-0
                                 ;;  set: 1 if angle folds from QII/QIII)
        DEFB    $E0             ;;get-mem-0             Y_sin, flag.
        DEFB    $30             ;;not                   Y_sin, notflag.
        DEFB    $00             ;;jump-true
        DEFB    NT_SKIP_NEG-$   ;;to NT_SKIP_NEG

        DEFB    $1B             ;;negate                Y_tan. (= -Y_sin)

NT_SKIP_NEG:
        DEFB    $C0             ;;st-mem-0              Y_tan.  (stash;
                                 ;;  get-argt's own flag no longer needed)
        DEFB    $31             ;;duplicate             Y_tan, Y_tan.
        DEFB    $2A             ;;abs                   Y_tan, absYtan.
        DEFB    $A2             ;;stk-half              Y_tan, absYtan, .5
        DEFB    $03             ;;subtract              Y_tan, Z. (absYtan-.5)
        DEFB    $37             ;;greater-0             Y_tan, (Z>0?1:0).
        DEFB    $00             ;;jump-true
        DEFB    NT_RECIP_PREP-$ ;;to NT_RECIP_PREP  (|Y_tan| > 0.5)

; --- DIRECT branch: |Y_tan| <= 0.5, x = Y_tan*(pi/2) is safely inside
; the Pade approximant's accurate range. Stack here: [Y_tan].
        DEFB    $A3             ;;stk-pi/2              Y_tan, pi/2.
        DEFB    $04             ;;multiply              x.
        DEFB    $A0             ;;stk-zero              x, 0.
        DEFB    $C2             ;;st-mem-2              x, 0.  (is_recip=0)
        DEFB    $02             ;;delete                x.
        DEFB    $33             ;;jump
        DEFB    NT_PADE-$       ;;to NT_PADE

; --- RECIP branch: |Y_tan| > 0.5. Use tan(x) = sgn(Y_tan)/tan(pi/2-|x|),
; where pi/2-|x| = (1-absYtan)*(pi/2) is safely inside Pade's range.
; Stack here (after jump-true consumed the test flag): [Y_tan].
NT_RECIP_PREP:
        DEFB    $2A             ;;abs                   absYtan.
        DEFB    $A1             ;;stk-one               absYtan, 1.
        DEFB    $01             ;;exchange              1, absYtan.
        DEFB    $03             ;;subtract              yprime. (1-absYtan)
        DEFB    $A3             ;;stk-pi/2              yprime, pi/2.
        DEFB    $04             ;;multiply              xprime.
        DEFB    $A1             ;;stk-one               xprime, 1.
        DEFB    $C2             ;;st-mem-2              xprime, 1. (is_recip=1)
        DEFB    $02             ;;delete                xprime.
                                 ; falls straight through into NT_PADE

; --- Shared Pade [3/2] evaluation: tan(x) ~= x(15-x^2)/(15-6x^2).
; Stack: [x].
NT_PADE:
        DEFB    $31             ;;duplicate             x, x.
        DEFB    $31             ;;duplicate             x, x, x.
        DEFB    $04             ;;multiply              x, x^2.
        DEFB    $31             ;;duplicate             x, x^2, x^2.
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $40             ;;(value 6.0)
        DEFB    $04             ;;multiply              x, x^2, 6x^2.
        DEFB    $34             ;;stk-data
        DEFB    $34             ;;Exponent: $84, Bytes: 1
        DEFB    $70             ;;(value 15.0)
        DEFB    $01             ;;exchange              x, x^2, 15, 6x^2.
        DEFB    $03             ;;subtract              x, x^2, denom.
        DEFB    $C1             ;;st-mem-1              x, x^2, denom.
        DEFB    $02             ;;delete                x, x^2.
        DEFB    $34             ;;stk-data
        DEFB    $34
        DEFB    $70             ;;(value 15.0)
        DEFB    $01             ;;exchange              x, 15, x^2.
        DEFB    $03             ;;subtract              x, numerfactor. (15-x^2)
        DEFB    $04             ;;multiply              numer. (x*(15-x^2))
        DEFB    $E1             ;;get-mem-1             numer, denom.
        DEFB    $05             ;;division              result. = pade_tan(x)

        DEFB    $E2             ;;get-mem-2             result, is_recip.
        DEFB    $00             ;;jump-true
        DEFB    NT_APPLY_RECIP-$;;to NT_APPLY_RECIP

        DEFB    $38             ;;end-calc              result.
        RET

NT_APPLY_RECIP:
        DEFB    $A1             ;;stk-one               result, 1.
        DEFB    $01             ;;exchange              1, result.
        DEFB    $05             ;;division              1/result.
        DEFB    $E0             ;;get-mem-0             1/result, Y_tan.
        DEFB    $29             ;;sgn                   1/result, sign(Y_tan).
        DEFB    $04             ;;multiply              final.
        DEFB    $38             ;;end-calc              final.
        RET

        DEFB    $00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00      ; 11 bytes of slack (3-byte header +
                                 ; 67-byte core in an 81-byte slot) --
                                 ; see Entry 2.
        ORG     $2382           ; safeguard DRAW, unmoved
...
; --- PATCHED (see Decisions.md): was "JP NC,L2684" (S-ALPHNUM) directly.
; Now routes through ADDR_DISPATCH first, which recognizes the reclaimed
; LPRINT token ($E0) as the new ADDR(var) function and falls through to
; the real S-ALPHNUM (L2684, unchanged) for every other character
; exactly as before. Same 3-byte JP cc,nn shape, so nothing else moves.
        JP      NC,ADDR_DISPATCH ; jump forward to S-ALPHNUM if not in primary
...
;; S-RND
; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 7): NEW_S_RND
; (29 bytes) inlined directly into its own original 47-byte slot,
; replacing the Checkpoint Alpha JP-trampoline-to-relocated-core design
; (same move as Entries 3/4/6's EXP/ATN/LN relocations). S-RND is
; entered as fresh Z80 code via the scan-func dispatch table (its own
; first instruction is a plain CALL, not RST 28H or a calculator
; literal), so no end-calc-first trick is needed -- execution just
; falls straight into the routine's own first CALL. This reclaims the
; 29 bytes NEW_S_RND previously occupied in the main free block, at the
; cost of 18 wasted bytes locally (see Entry 2's best-fit analysis).
        ORG     $25F8           ; original S-RND entry point -- verified
                                 ; against baseline.bin before patching
L25F8:
        CALL    $2530           ; SYNTAX-Z
        JR      Z,RND_SYNTAX_SKIP  ; checking syntax -- original also
                                    ; skips all calculator interaction
                                    ; here, replicated exactly
        CALL    NEW_RND_BYTE
        PUSH    AF              ; NEW_RND_BYTE uses B internally as
                                ; scratch, so the first byte can't be
                                ; left sitting in B across the second
                                ; call -- saved via the stack instead
                                ; (see Alpha Decisions.md for the bug
                                ; this fixed).
        CALL    NEW_RND_BYTE
        LD      C,A
        POP     AF
        LD      B,A
        CALL    $2D2B           ; STACK-BC
        RST     28H             ;; FP-CALC              raw16.
        DEFB    $34             ;;stk-data
        DEFB    $39             ;;Exponent: $89, Bytes: 1
        DEFB    $00             ;;(value 256.0)          raw16, 256.
        DEFB    $31             ;;duplicate              raw16, 256, 256.
        DEFB    $04             ;;multiply               raw16, 65536.
                                 ;; (built from 256*256, not a single
                                 ;; stk-data constant -- 65536 overflows
                                 ;; the header byte's 6-bit exponent
                                 ;; field; see Alpha Decisions.md)
        DEFB    $05             ;;division              raw16/65536.
        DEFB    $38             ;;end-calc
RND_SYNTAX_SKIP:
        JP      $2630           ; S-PI-END
        ...
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00           ; 18 bytes of slack (29-byte routine
                                   ; in a 47-byte slot) -- see Entry 2.
        ORG     $2627           ; safeguard S-PI, unmoved
...
;; EXP
;; exp
; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 3): NEW_EXP_CORE
; (52 bytes) inlined directly into its own original 53-byte slot,
; replacing the Checkpoint Alpha JP-trampoline-to-relocated-core design.
; No JP needed at all -- execution simply falls into the routine at the
; slot's own entry address, exactly as the original 'exp' did. This
; reclaims the 52 bytes NEW_EXP_CORE previously occupied in the main
; free block ($3A62-$3A95), at the cost of 1 wasted byte locally (see
; Entry 2's best-fit analysis). Everything below "CALL L2DD5" (FP-TO-A
; onward -- the raw Z80 tail that converts the integer exponent
; adjustment k into a direct exponent-byte addition, plus all of its
; overflow/sign error handling) is completely untouched and reused
; unchanged, exactly as in Checkpoint Alpha: the routine ends its own
; calculator program with the exact same stack convention the original
; left (pade_result, k) and jumps straight into that same address,
; $36F9, which is unmoved.
; --- PATCHED (see checkpoint_ZXROMEX_v1.1 Decisions.md, EXP Pade[3/3]
; revision): the Beta-era inlined Pade[2/2] core is REMOVED and replaced
; with a JP trampoline to NEW_EXP_CORE_V2 (Pade[3/3], relocated into the
; main free block at $3ACD). This is a REVERT to the Alpha-era trampoline
; shape, not a new design -- Pade[3/3]'s 85-byte body no longer fits the
; original 53-byte inline slot the way Pade[2/2]'s 52 bytes did.
;
; Root cause being fixed: Pade[2/2] (6.99e-6 worst-case relative error,
; verified) is well within its own documented bound, but that bound turned
; out to be too loose for `^` on real 4-digit integers -- 5000^2 came out
; 76.94 off via the LN->EXP composition. Pade[3/3] (~3.3e-8 worst-case
; relative error, see NEW_EXP_CORE_V2 below) fixes this at the EXP layer;
; the `to-power` integer fast-path patch below fixes the common case
; (5000^2 itself) at the `^` layer by avoiding LN/EXP entirely for
; whole-number exponents. Both together, not either alone -- `^` with a
; non-integer Y still goes through this fixed EXP core.
        ORG     $36C4           ; original EXP entry point -- verified
                                 ; against baseline.bin before patching
L36C4:  JP      NEW_EXP_CORE_V2
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00
                                 ; 3-byte JP + 49 zero bytes, filling the
                                 ; original 53-byte slot exactly.
        ORG     $36F9           ; safeguard the reused tail, unmoved
...
L373D:  DEFB    $01             ;;exchange
        ...
        DEFB    $A2             ;;stk-half
        DEFB    $03             ;;subtract
; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 6): NEW_LN_TAIL
; (36 bytes) inlined directly into its own original 57-byte slot,
; replacing the Checkpoint Alpha JP-trampoline-to-relocated-core design
; (same move as Entries 3/4's EXP/ATN relocations). Only the polynomial
; evaluation is replaced (everything from here onward in the original);
; the reduction above this point (exponent split, then this M>0.8
; threshold check that further tightens the range) is completely
; untouched. Reached as a continuing literal stream (GRE.8 itself is
; entered via calculator-level jump-true or RST28H-fresh-fallthrough,
; both already-open contexts by the time this point is reached), so
; end-calc is still required first; but no Z80 JP is needed at all
; after it -- execution just falls straight into the routine's own
; fresh RST 28H, the same pattern Entries 3/4 already used. This
; reclaims the 36 bytes NEW_LN_TAIL previously occupied in the main
; free block, at the cost of 20 wasted bytes locally (see Entry 2's
; best-fit analysis).
        ORG     $374A           ; original insertion point -- verified
                                 ; against baseline.bin before patching
L374A:  DEFB    $38             ;;end-calc

; --- NEW_LN_TAIL -- 3-term atanh-series LN replacement. Entry:
; [k*ln2, u] on the FP calculator stack (u = M-1, from the unmodified
; reduction above). Exit: k*ln2 + ln(M) = ln(x), matching the
; original's own final "addition" exactly.
;
; ln(M) ~= 2y(1 + y^2/3 + y^4/5), y = u/(u+2) -- max abs error 1.04e-5
; (see Alpha Decisions.md for full derivation and verification against
; the actual reduction range this produces, |u| <= 0.6).
;
; No calculator memory used, no collision risk -- pure stack arithmetic.
NEW_LN_TAIL:
        RST     28H             ;; FP-CALC              k*ln2, u.
        DEFB    $31             ;;duplicate             k*ln2, u, u.
        DEFB    $34             ;;stk-data
        DEFB    $32             ;;Exponent: $82, Bytes: 1
        DEFB    $00             ;;(value 2.0)
        DEFB    $0F             ;;addition              k*ln2, u, u+2.
        DEFB    $05             ;;division              k*ln2, y. (u/(u+2))

        DEFB    $31             ;;duplicate             k*ln2, y, y.
        DEFB    $31             ;;duplicate             k*ln2, y, y, y.
        DEFB    $04             ;;multiply              k*ln2, y, y^2.
        DEFB    $31             ;;duplicate             k*ln2, y, t, t.
        DEFB    $34             ;;stk-data
        DEFB    $EE             ;;Exponent: $7E, Bytes: 4
        DEFB    $4C,$CC,$CC,$CD ;;(value 0.2)
        DEFB    $04             ;;multiply              k*ln2, y, t, 0.2t.
        DEFB    $34             ;;stk-data
        DEFB    $EF             ;;Exponent: $7F, Bytes: 4
        DEFB    $2A,$AA,$AA,$AB ;;(value 1/3)
        DEFB    $0F             ;;addition              k*ln2, y, t, 1/3+0.2t.
        DEFB    $04             ;;multiply              k*ln2, y, t*(1/3+0.2t).
        DEFB    $A1             ;;stk-one               k*ln2, y, ..., 1.
        DEFB    $0F             ;;addition              k*ln2, y, 1+t*(...).
        DEFB    $04             ;;multiply              k*ln2, y*(1+...).
        DEFB    $34             ;;stk-data
        DEFB    $32             ;;Exponent: $82, Bytes: 1
        DEFB    $00             ;;(value 2.0)
        DEFB    $04             ;;multiply              k*ln2, ln(M).

        DEFB    $0F             ;;addition              k*ln2 + ln(M).
        DEFB    $38             ;;end-calc
        ...
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00
        DEFB    $00,$00,$00,$00 ; 20 bytes of slack (36-byte routine,
                                 ; plus this slot's own 1-byte end-calc,
                                 ; in a 57-byte slot) -- see Entry 2.
        ORG     $3783           ; safeguard get-argt, unmoved
...
; ---------------------
; (offset $20: 'cos')
; NOTE: the paragraph below describes the ORIGINAL, pre-patch approach
; (sine of the complementary angle) for historical reference. See the
; "PATCHED" comment just above ";; cos" for what this ROM actually does
; now -- an independent Bhaskara I formula, not routed through sine.
...
;; cos
; --- PATCHED (see Alpha Decisions.md and checkpoint_BETA_Decisions.md
; Entry 5): trampoline to NEW_COS_CORE, an independent Bhaskara I
; cosine approximation -- no longer computed as "sine of the
; complementary angle" via the shared C-ENT code. As of Entry 5,
; NEW_COS_CORE itself lives inside C-ENT's own spare trampoline padding
; ($37BB) rather than the main free block -- this JP's target moved,
; but the trampoline here is otherwise unchanged: reached only via the
; calculator's table dispatch, so a plain Z80 JP suffices, no end-calc
; needed first.
        ORG     $37AA           ; original cos entry point -- verified
                                 ; against baseline.bin before patching
L37AA:  JP      NEW_COS_CORE

; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 9):
; RND_DEFAULT_TABLE (8 bytes of pure data, the CMWC generator's default
; seed table) relocated here, into COS's own spare trampoline padding
; -- a perfect zero-waste fit, the tightest of any relocation in this
; project (3-byte header + 8-byte table = 11 bytes, exactly filling the
; 11-byte slot). Referenced only via "LD HL,RND_DEFAULT_TABLE" from
; RND_BOOT_SEED and RND_RESEED (absolute addressing, no proximity
; dependency), so relocating it here is safe. This reclaims the 8 bytes
; RND_DEFAULT_TABLE previously occupied in the main free block, and
; closes out Entry 2's original repacking worklist.
RND_DEFAULT_TABLE:
        DEFB    82,97,120,111,102,116,20,12
        ORG     $37B5           ; safeguard sin, unmoved
...
;; C-ENT
; --- PATCHED (see Decisions.md): trampoline to NEW_SIN_CORE, a Bhaskara I
; approximation replacing the original Chebyshev-polynomial (series-06)
; evaluation. Reached two ways -- falling straight through from 'sin'
; above (same outer RST 28H invocation, no end-calc between get-argt and
; here in the original code), and via a calculator-level jump/jump-true
; from 'cos' below, which can only reach a nearby literal-stream address.
; So this trampoline itself must still be valid AT THIS EXACT ADDRESS as
; literal-stream content: end-calc first (cleanly unwinds whichever outer
; invocation reached here), THEN a Z80 JP to the relocated routine (which
; opens its own fresh RST 28H, exactly the pattern used inside NEW_SQR).
        ORG     $37B7           ; original C-ENT entry point -- verified
                                 ; against baseline.bin before patching
L37B7:  DEFB    $38             ;;end-calc
        JP      NEW_SIN_CORE

; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 5): NEW_COS_CORE
; (30 bytes) relocated here, into C-ENT's own spare padding, rather than
; sitting in the main free block. This slot's own required header
; (end-calc + JP NEW_SIN_CORE, 4 bytes) is unrelated to and unaffected
; by this -- NEW_COS_CORE is reached only via a separate JP from COS's
; own trampoline at $37AA, which is never fallen-into from here (this
; code is only reachable by that external jump), so storing it in the
; bytes following C-ENT's own JP is safe: those bytes are never executed
; as a fall-through, only jumped to directly. 1 byte of slack remains in
; the 35-byte slot (4 header + 30 core + 1 slack). This reclaims the 30
; bytes NEW_COS_CORE previously occupied in the main free block
; ($38CB-$38E8), right after NEW_SIN_CORE.
;
; NEW_COS_CORE -- independent Bhaskara I cosine approximation. Reached
; only via the trampoline left at L37AA (table dispatch only -- no
; calculator-level jump targets this address). Entry: angle in radians
; on top of the FP calculator stack. Exit: cos(angle) on top of the FP
; calculator stack, RET.
;
; Derivation: substituting x -> pi/2 - x into Bhaskara's sine formula
; and simplifying gives cos(x) ~= (pi^2 - 4x^2)/(pi^2 + x^2) for
; x in [-pi/2, pi/2]. Since Y = x/(pi/2) (get-argt's output), substituting
; x = Y*pi/2 and simplifying cancels pi entirely:
;   cos(Y*pi/2) ~= 4(1-Y^2)/(4+Y^2)
; This formula alone is only correct for the QI/QIV portion of get-argt's
; folding (matching sine's own periodicity, not cosine's) -- the
; magnitude is always right but the sign is wrong exactly when
; get-argt's own quadrant flag (mem-0, a side effect of calling it) is
; set, so that flag is reused directly to correct the sign, with no
; separate cosine-specific quadrant logic needed. See Alpha Decisions.md
; for full derivation and verification.
;
; Uses calculator memory 1 as scratch (mem-0 is read, not written, since
; it holds get-argt's own quadrant flag which this routine needs intact
; until the final sign check).
NEW_COS_CORE:
        RST     28H             ;; FP-CALC              angle.
        DEFB    $39             ;;get-argt              Y.  (mem-0 set as
                                 ;;  a side effect: 1 if the angle folds
                                 ;;  from quadrant II/III, 0 otherwise --
                                 ;;  used below for sign correction.)
        DEFB    $31             ;;duplicate             Y, Y.
        DEFB    $04             ;;multiply              Y^2.
        DEFB    $31             ;;duplicate             Y^2, Y^2.
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $00             ;;(value 4.0)
        DEFB    $0F             ;;addition              Y^2, denom. (4+Y^2)
        DEFB    $C1             ;;st-mem-1              Y^2, denom.
        DEFB    $02             ;;delete                Y^2.
        DEFB    $34             ;;stk-data
        DEFB    $33
        DEFB    $00             ;;(value 4.0)
        DEFB    $04             ;;multiply              4*Y^2.
        DEFB    $34             ;;stk-data
        DEFB    $33
        DEFB    $00             ;;(value 4.0)
        DEFB    $01             ;;exchange              4, 4Y^2.
        DEFB    $03             ;;subtract              numer. (4-4Y^2)
        DEFB    $E1             ;;get-mem-1             numer, denom.
        DEFB    $05             ;;division              result. (unsigned
                                 ;;  magnitude, correct for QI/QIV as-is)
        DEFB    $E0             ;;get-mem-0             result, flag.
        DEFB    $00             ;;jump-true
        DEFB    NEGATE-$        ;;to NEGATE (QII/QIII: sign flip needed)

        DEFB    $38             ;;end-calc              result.
        RET

NEGATE:
        DEFB    $1B             ;;negate                result.
        DEFB    $38             ;;end-calc              result.
        RET

        DEFB    $00             ; 1 byte of slack (4-byte header + 30-byte
                                 ; core in a 35-byte slot) -- see Entry 2.
        ORG     $37DA           ; safeguard tan, unmoved
...
;; tan
; --- PATCHED (see Alpha Decisions.md and checkpoint_BETA_Decisions.md
; Entry 8): trampoline to NEW_TAN_CORE, an independent Pade [3/2]
; rational approximant -- no longer computed as sin(x)/cos(x). As of
; Entry 8, NEW_TAN_CORE itself lives inside CIRCLE's own spare
; trampoline padding ($2334) rather than the main free block -- this
; JP's target moved, but the trampoline here is otherwise unchanged:
; reached only via table dispatch, so a plain Z80 JP suffices.
        ORG     $37DA           ; original tan entry point -- verified
                                 ; against baseline.bin before patching
L37DA:  JP      NEW_TAN_CORE
        DEFB    $00,$00,$00,$00,$00
        ORG     $37E2           ; safeguard atn, unmoved
...
;; CASES
; --- PATCHED (see checkpoint_BETA_Decisions.md, Entry 4): NEW_ATN_CORE
; (51 bytes) inlined directly into its own original 57-byte slot,
; replacing the Checkpoint Alpha JP-trampoline-to-relocated-core design
; (same move as Entry 3's EXP relocation). Reached both by falling
; straight through from SMALL (its own RST 28H, no end-calc between it
; and here) and via a calculator-level jump/jump-true from the BIG-case
; preprocessing above -- same situation as SIN/COS's C-ENT trampoline,
; so end-calc is still required first; but no Z80 JP is needed at all
; after it -- execution just falls straight into the routine's own
; fresh RST 28H, exactly the pattern NEW_SQR already used from a
; Z80-level JP target. This reclaims the 51 bytes NEW_ATN_CORE
; previously occupied in the main free block ($3969-$399B), at the cost
; of 5 wasted bytes locally (see Entry 2's best-fit analysis).
        ORG     $37FA           ; original CASES entry point -- verified
                                 ; against baseline.bin before patching
L37FA:  DEFB    $38             ;;end-calc

; --- NEW_ATN_CORE -- continued-fraction ATN approximation. Entry:
; [offset, t] on the FP calculator stack, where t is either -1/x (for
; |x|>=1, reciprocal-transformed by the untouched preprocessing above)
; or x directly (for |x|<1), and offset is 0, +pi/2, or -pi/2
; accordingly -- exactly the original CASES's own contract, unchanged.
; Exit: offset+atan(t) = atan(x), matching the original's own final
; "addition" exactly.
;
; atan(t) ~= t / (1 + t^2/(3 + 4t^2/(5 + 9t^2/(7 + 16t^2/9)))) -- a
; depth-4 continued fraction, max abs error 1.87e-4 (see Alpha
; Decisions.md for the full derivation and verification).
;
; Uses calculator memory 0 (t^2, held across all four levels of the
; continued fraction). No collision risk, as established in Alpha.
        RST     28H             ;; FP-CALC              t, offset.
        DEFB    $01             ;;exchange              offset, t.  (this
                                 ;;  is the original CASES's own first
                                 ;;  instruction -- forgotten on the
                                 ;;  first pass, see Alpha Decisions.md)
        DEFB    $31             ;;duplicate             offset, t, t.
        DEFB    $31             ;;duplicate             offset, t, t, t.
        DEFB    $04             ;;multiply              offset, t, t^2.
        DEFB    $C0             ;;st-mem-0              offset, t, t^2.
        DEFB    $02             ;;delete                offset, t.

; level 4: val4 = 16t^2/9
        DEFB    $E0             ;;get-mem-0             offset, t, t^2.
        DEFB    $34             ;;stk-data
        DEFB    $35             ;;Exponent: $85, Bytes: 1
        DEFB    $00             ;;(value 16.0)
        DEFB    $04             ;;multiply              offset, t, 16t^2.
        DEFB    $34             ;;stk-data
        DEFB    $34             ;;Exponent: $84, Bytes: 1
        DEFB    $10             ;;(value 9.0)
        DEFB    $05             ;;division              offset, t, val4.

; level 3: val3 = 9t^2/(7+val4)
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $60             ;;(value 7.0)
        DEFB    $0F             ;;addition              offset, t, 7+val4.
        DEFB    $E0             ;;get-mem-0             offset, t, denom3, t^2.
        DEFB    $34             ;;stk-data
        DEFB    $34             ;;Exponent: $84, Bytes: 1
        DEFB    $10             ;;(value 9.0)
        DEFB    $04             ;;multiply              offset, t, denom3, 9t^2.
        DEFB    $01             ;;exchange              offset, t, 9t^2, denom3.
        DEFB    $05             ;;division              offset, t, val3.

; level 2: val2 = 4t^2/(5+val3)
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $20             ;;(value 5.0)
        DEFB    $0F             ;;addition              offset, t, 5+val3.
        DEFB    $E0             ;;get-mem-0             offset, t, denom2, t^2.
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $00             ;;(value 4.0)
        DEFB    $04             ;;multiply              offset, t, denom2, 4t^2.
        DEFB    $01             ;;exchange              offset, t, 4t^2, denom2.
        DEFB    $05             ;;division              offset, t, val2.

; level 1: val1 = t^2/(3+val2)
        DEFB    $34             ;;stk-data
        DEFB    $32             ;;Exponent: $82, Bytes: 1
        DEFB    $40             ;;(value 3.0)
        DEFB    $0F             ;;addition              offset, t, 3+val2.
        DEFB    $E0             ;;get-mem-0             offset, t, denom1, t^2.
        DEFB    $01             ;;exchange              offset, t, t^2, denom1.
        DEFB    $05             ;;division              offset, t, val1.

; final: atan(t) = t/(1+val1)
        DEFB    $A1             ;;stk-one               offset, t, val1, 1.
        DEFB    $0F             ;;addition              offset, t, 1+val1.
        DEFB    $05             ;;division              offset, atan(t).

        DEFB    $0F             ;;addition              offset+atan(t).
        DEFB    $38             ;;end-calc

        RET

        DEFB    $00,$00,$00,$00,$00 ; 5 bytes of slack (51-byte routine,
                                 ; plus this slot's own 1-byte end-calc,
                                 ; in a 57-byte slot) -- see Entry 2.
        ORG     $3833           ; safeguard asn, unmoved
...
;; sqr
; --- PATCHED (see Decisions.md): trampoline to NEW_SQR, a Newton-Raphson
; replacement placed in the ROM's free space at $386E. Behavior (result,
; SQR 0 = 0, negative -> Report A) is unchanged; only the algorithm and
; speed differ. The original 7-byte body started here and ran through to
; the byte immediately before L3851 (to-power, untouched) -- this
; trampoline reoccupies exactly those same 7 bytes.
        ORG     $384A           ; original sqr entry point -- verified
                                 ; against baseline.bin before patching
L384A:  JP      NEW_SQR
        DEFB    $00,$00,$00,$00 ; pad out to the original 7-byte width
        ORG     $3851           ; safeguard L3851 (to-power), unmoved
...
; --- PATCHED (see ZXROMEX v1.1 Decisions.md, `^` integer-exponent fast
; path): only the 12-byte HEAD ($3851-$385C) is replaced with a trampoline
; to TO_POWER_V2 (in the main free block). XIS0/ONE/LAST below ($385D-
; $386D) are left byte-identical and completely unreferenced by this new
; head -- any other-language/assembly caller jumping directly to $385D
; still gets identical, original behavior. TO_POWER_V2 reaches the same
; 0^anything semantics via its own local copy, XIS0_LOCAL, not by jumping
; back here (a calculator jump-true can't cross the end-calc/relocation
; this design needs -- see XIS0_LOCAL's own header for why).
;
; Motivation: 5000^2 should never need LN+EXP at all -- it's exact
; repeated multiplication. TO_POWER_V2 detects a whole-number Y in
; [-127,127] and uses exponentiation-by-squaring instead (SQ_LOOP_ENTRY),
; costing log2(n) multiplies with error only from log2(n) roundings.
; Non-integer or out-of-range Y falls through to the ORIGINAL ln/multiply/
; end-calc/JP-to-EXP sequence, byte-identical to what to-power always did
; -- now hitting the fixed Pade[3/3] EXP core above.
        ORG     $3851           ; to-power's own entry -- unmoved, unchanged
L3851:  JP      TO_POWER_V2
        DEFB    $00,$00,$00,$00,$00,$00,$00,$00,$00
                                 ; 3-byte JP + 9 zero bytes, filling the
                                 ; original 12-byte head exactly.
        ORG     $385D           ; safeguard XIS0/ONE/LAST, byte-identical,
                                 ; untouched, still independently reachable
...
;; sqr (NEW_SQR -- Newton-Raphson replacement, see Decisions.md)
; Reached only via the JP trampoline left at L384A. Entry/exit contract is
; identical to the routine it replaces: entered with x already on top of
; the FP calculator stack, returns with the result on top of the FP
; calculator stack (or raises Report A for negative x, matching the
; original's own delegation to ln's negative check).
;
; Verified standalone (pasmo, see Decisions.md) at exactly 57 bytes,
; comfortably inside the 1170-byte free block that starts here.
        ORG     $386E           ; genuine free-space start -- verified
                                 ; against baseline.bin before patching
L386E:
NEW_SQR:
        RST     28H             ;; FP-CALC              x.
        DEFB    $3D             ;;re-stack              x.  (normalize a
                                 ;;  short-integer-format argument to full
                                 ;;  floating-point form first -- BASIC
                                 ;;  stores small integer literals/values
                                 ;;  this way, and NS_POS below reads the
                                 ;;  exponent byte directly, so it must be
                                 ;;  a real exponent, not a short-int
                                 ;;  marker. Mirrors ln's own first step.)
        DEFB    $31             ;;duplicate             x, x.
        DEFB    $30             ;;not                   x, (x==0?1:0).
        DEFB    $00             ;;jump-true
        DEFB    NS_ZERO-$       ;;to NS_ZERO

        DEFB    $31             ;;duplicate             x, x.
        DEFB    $37             ;;greater-0             x, (x>0?1:0).
        DEFB    $00             ;;jump-true
        DEFB    NS_POS-$        ;;to NS_POS

        DEFB    $38             ;;end-calc              x   (negative)
        JP      L371A           ; -> REPORT-Ab "Invalid argument" (ln's own
                                 ; error path, reused rather than duplicated)

NS_ZERO:
        DEFB    $38             ;;end-calc              x   (x is 0)
        RET

NS_POS:
        DEFB    $38             ;;end-calc              x   (x > 0; HL -> x's
                                 ;;  exponent byte, same guarantee end-calc
                                 ;;  always leaves -- see ln's VALID path)
        RST     28H             ;; FP-CALC              x.
        DEFB    $31             ;;duplicate             x, x.
        DEFB    $38             ;;end-calc              x, x  (HL -> dup's
                                 ;;  exponent byte)

        LD      A,(HL)          ; A = exponent (same value as x's -- the
                                 ; duplicate is a verbatim copy so far)
        XOR     $80             ; halve the exponent (fSQRT.bas's own guess
        SRA     A               ; heuristic, ported byte-for-byte): toggle
        INC     A               ; sign, shift right (sign unchanged), +1,
        JR      Z,NS_ASIS        ; and re-decrement unless that pushed us
        JP      P,NS_ASIS        ; from just-below-zero to just-above.
        DEC     A
NS_ASIS:
        XOR     $80             ; restore sign -> A = guess exponent
        LD      (HL),A          ; overwrite the DUPLICATE's exponent in
                                 ; place -- same mantissa as x, new exponent
                                 ; -- so the duplicate is now the guess.

        RST     28H             ;; FP-CALC              x, guess.
        DEFB    $C3             ;;st-mem-3              x, guess.
        DEFB    $02             ;;delete                x.

; ---- fSQRT.bas's own Newton-Raphson loop, byte-for-byte identical ----
NS_LOOP:
        DEFB    $31             ;;duplicate
        DEFB    $E3             ;;get-mem-3
        DEFB    $C4             ;;st-mem-4
...
; ---------------------
; NEW_SIN_CORE -- Bhaskara I sine approximation, see Decisions.md.
; Reached only via the trampoline left at L37B7 (both sin's fall-through
; and cos's calculator-level jump/jump-true land there). Entry: Y (the
; get-argt-reduced angle, range -1..+1) on top of the FP calculator
; stack. Exit: sin(Y*pi/2) on top of the FP calculator stack, RET.
; No raw exponent-byte manipulation is used (unlike NEW_SQR's guess
; step), so no re-stack is needed -- every op here (duplicate, sgn,
; multiply, abs, subtract, addition, division, exchange, st/get-mem) is
; a generic calculator operation that already handles both the short-
; integer and full-float representations transparently.
;
; Verified standalone (pasmo, see Decisions.md) at exactly 36 bytes.
;
; Uses calculator memories 0 and 1 as scratch. Confirmed safe: the ROM's
; own comments at the DRAW/CIRCLE call sites (search "trash locations
; mem-0 to mem-2") already document that sin/cos may not preserve mem-0
; through mem-2 across a call, and every caller in the ROM is already
; written accordingly -- see Decisions.md for the full audit.
; ---------------------
        ORG     $38A7           ; immediately after NEW_SQR -- verified
                                 ; against baseline.bin before patching
NEW_SIN_CORE:
        RST     28H             ;; FP-CALC              Y.
        DEFB    $31             ;;duplicate             Y, Y.
        DEFB    $29             ;;sgn                   Y, s.
        DEFB    $C0             ;;st-mem-0              Y, s.   (stash sign)
        DEFB    $02             ;;delete                Y.
        DEFB    $A2             ;;stk-half              Y, 0.5.
        DEFB    $04             ;;multiply              w.      (w = Y/2)
        DEFB    $2A             ;;abs                   absW.
        DEFB    $31             ;;duplicate             absW, absW.
        DEFB    $31             ;;duplicate             absW, absW, absW.
        DEFB    $04             ;;multiply              absW, absW^2.
        DEFB    $03             ;;subtract              u.      (absW-absW^2)
        DEFB    $31             ;;duplicate             u, u.
        DEFB    $31             ;;duplicate             u, u, u.
        DEFB    $31             ;;duplicate             u, u, u, u.
        DEFB    $0F             ;;addition              u, u, 2u.
        DEFB    $0F             ;;addition              u, 3u.
        DEFB    $0F             ;;addition              v.      (v = 4u)
        DEFB    $31             ;;duplicate             v, v.
        DEFB    $34             ;;stk-data
        DEFB    $33             ;;Exponent: $83, Bytes: 1
        DEFB    $20             ;;(value 5.0)
        DEFB    $01             ;;exchange              v, 5, v.
        DEFB    $03             ;;subtract              v, denom.  (5-v)
        DEFB    $C1             ;;st-mem-1              v, denom.
        DEFB    $02             ;;delete                v.
        DEFB    $31             ;;duplicate             v, v.
        DEFB    $0F             ;;addition              2v.
        DEFB    $31             ;;duplicate             2v, 2v.
        DEFB    $0F             ;;addition              numerator. (16u)
        DEFB    $E1             ;;get-mem-1             numerator, denom.
        DEFB    $05             ;;division              result.
        DEFB    $E0             ;;get-mem-0             result, s.
        DEFB    $04             ;;multiply              final.
        DEFB    $38             ;;end-calc              final.
...
; ---------------------
; NEW_COS_CORE has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 5: it now lives inside C-ENT's own spare trampoline padding at
; $37BB (see the 'C-ENT' section, and the 'cos' trampoline at $37AA),
; reclaiming these 30 bytes for the main free block. This comment marks
; where it used to sit, for anyone cross-referencing Checkpoint Alpha's
; own memory map against this build.
; ---------------------
; ---------------------
; NEW_TAN_CORE has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 8: it now lives inside CIRCLE's own spare trampoline padding at
; $2334 (see the CIRCLE trampoline at $2331, and the 'tan' trampoline
; at $37DA), reclaiming these 67 bytes for the main free block. This
; comment marks where it used to sit, for anyone cross-referencing
; Checkpoint Alpha's own memory map against this build.
; ---------------------
; ADDR_DISPATCH / S_ADDR -- the ADDR(var) function, see Decisions.md and
; the project's KEYWORD_ADDR.md specification. Hooked into S-LOOP-1's
; own fallback jump (see the edit near L24FF above) rather than reached
; via a trampoline the way the calculator-literal patches were -- this
; is plain Z80 code reached by a plain Z80 JP, no calculator-literal
; entry/exit convention involved at all.
;
; Reclaims LPRINT's token ($E0) for ADDR's mid-expression meaning only.
; LPRINT's own statement behavior (typed at the start of a line) is
; completely untouched -- this hook only fires during expression
; scanning, a different code path entirely.
;
; LPRINT (not COPY, the first choice -- see Decisions.md) specifically
; because LPRINT lives in the E-UNSHIFT keyboard table, the same one
; SIN/COS/PEEK use: typed via Extended Mode, which is available
; anywhere, including mid-expression. COPY is a K-mode-only token --
; K-mode is only offered by the editor at the start of a line, right
; after ':', or right after 'THEN' (confirmed directly in OUT-CHAR's
; source) -- so while COPY's *token*, once present in a tokenized
; program, dispatches to ADDR correctly, it could never actually be
; *typed* in an expression position like "PRINT COPY(x)" at the
; keyboard. LPRINT doesn't have that problem.
;
; ADDR(x) returns the address of a numeric variable or array element's
; underlying 5-byte float; ADDR(arr(1,1,...,1)) (all subscripts 1,
; matching the array's real dimensionality) returns the array's own
; base address, since that's the element whose computed offset is
; mathematically forced to zero. String variables/elements are a
; deliberate placeholder (REPORT-C) -- not implemented.
;
; Verified standalone (pasmo, see Decisions.md) at exactly 52 bytes.
; ---------------------
        ORG     $38CB           ; immediately after NEW_SIN_CORE --
                                 ; shifted down a further 67 bytes from
                                 ; Entry 5's $390E (Entry 8, NEW_TAN_CORE
                                 ; relocation). Total shift from
                                 ; Checkpoint Alpha's original $392C:
                                 ; 97 bytes (30+67).
LOOK_VARS       EQU $28B2
STK_VAR         EQU $2996
STACK_BC        EQU $2D2B
...
REPORT_2        EQU $1C2E
SYNTAX_Z        EQU $2530
ADDR_TOKEN      EQU $E0         ; LPRINT's token, being reclaimed
LLIST_TOKEN     EQU $E1         ; LLIST's token, being reclaimed (mid-
                                 ; expression meaning only -- LLIST's own
                                 ; statement form, typed at line-start,
                                 ; is untouched, same principle as LPRINT)
...
;; S-ADDR
S_ADDR:
        RST     20H             ; consume the token
        CP      $28             ; expect '('
        JP      NZ,REPORT_C
        RST     20H             ; consume '(' -> CH_ADD at the variable ref

        CALL    LOOK_VARS
        JP      C,REPORT_2      ; "Variable not found"
        CALL    Z,STK_VAR       ; only fires for array/subscript syntax

; --- FIX 1 of 2 (see Decisions.md): the original "LD A,($5C3B); CP $C0"
; string/numeric check tested FLAGS bit 7 (running-program) together
; with bit 6 (numeric-result) -- but bit 7 is always clear during
; syntax-checking, so that check treated EVERY variable as a string,
; and jumped to REPORT_C, during syntax-checking, regardless of the
; SYNTAX-Z fix below (which never even got reached). Test bit 6 alone --
; the bit LOOK_VARS/STK_VAR actually set/clear for this ("SET 6,(IY+$01)
; ; presume numeric result" / "RES 6,(IY+$01) ; signal string result",
; both confirmed directly in LOOK-VARS's own source) -- independent of
; runtime vs syntax-check mode.
        BIT     6,(IY+$01)      ; FLAGS bit 6: numeric(1) or string(0)?
        JR      Z,ADDR_STRING   ; Z=1 -> bit 6 clear -> string result

; --- FIX 2 of 2: during syntax-checking -- the automatic pass that runs
; on a typed line the moment ENTER is pressed, before the line is even
; stored -- LOOK_VARS/STK_VAR do NOT perform a real search; HL is never
; set to a meaningful address there (LOOK_VARS's own comment says so
; outright: "if checking syntax the letter is not returned"). An
; earlier version of this routine trusted HL unconditionally, which
; meant a perfectly valid line using ADDR could fail to even be
; accepted by the editor, though it worked fine once loaded from tape
; (which bypasses this syntax-check pass entirely). Check SYNTAX-Z
; (same routine LOOK_VARS itself uses for exactly this purpose) and
; fall back to a safe placeholder (BC=0) instead of touching HL
; whenever only checking syntax.
        CALL    SYNTAX_Z
        JR      Z,SA_SYNTAX     ; Z=1 -> checking syntax only
        INC     HL              ; the single correction (real address)
        LD      B,H
        LD      C,L
        JR      SA_STACK
SA_SYNTAX:
        LD      BC,$0000        ; safe placeholder -- value is discarded,
                                 ; this line will be re-scanned for real
                                 ; the moment it actually runs
SA_STACK:
        CALL    STACK_BC        ; stacks BC as a numeric result
        JR      ADDR_DONE
...
ADDR_BLOCK_END:
; Marker label, same self-correcting-ORG discipline used throughout this
; revision -- gives the SAFE_ZERO_BYTE anchor below something to be
; COMPUTED FROM rather than a stale hardcoded literal.

; ---------------------
; NEW_ATN_CORE has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 4: it now lives inlined at $37FA, inside its own original
; CASES trampoline slot (see the 'CASES' section above), eliminating
; the JP that used to reach it here and reclaiming these 51 bytes for
; the main free block. This comment marks where it used to sit, for
; anyone cross-referencing Checkpoint Alpha's own memory map against
; this build.
; ---------------------
; ---------------------
; SAFE_ZERO_BYTE / CL_PRINTER_FIX -- see Alpha Decisions.md "Patch 7". A
; single guaranteed-zero byte (not a buffer -- every site that used to
; walk a range of the real printer buffer has been individually patched
; to stay fixed on this one address instead, see the edits at CL-SET,
; COPY-BUFF/COPY-L-3, and CLEAR-PRB above). Confirmed via an exhaustive
; search of the whole ROM source that CL-SET, COPY-BUFF, and CLEAR-PRB
; are the *only* three literal references to $5B00 anywhere; PR_CC
; needs no separate handling since it only ever stores whatever CL-SET
; last computed.
;
; CL_PRINTER_FIX exists because CL-SET's own column-offset addition
; (shared with the screen-positioning path, so it can't just be deleted)
; would otherwise walk HL up to 32 bytes away from SAFE_ZERO_BYTE for
; the printer case. Re-tests the same printer-in-use flag CL-SET itself
; already tested, and forces HL back to SAFE_ZERO_BYTE only when the
; printer path was taken -- the screen case passes through to PO-STORE
; completely unaffected, byte-for-byte as before.
;
; Real Z80 writes to ROM addresses (below 16384) are a documented no-op
; on real hardware -- confirmed directly against Fuse during this
; project's very first patch -- so this byte can never become non-zero
; by construction. PRINT #3/LIST #3/COPY/LPRINT/LLIST all still run
; without error (a silent no-op, not a crash or syntax change); they
; just produce no visible output and no longer touch PRBUFF at all --
; freeing all 256 bytes of it for Patch 8 (RND)'s persistent state.
;
; Verified standalone (pasmo, see Decisions.md) at exactly 13 bytes
; (1 + 12).
; ---------------------
        ORG     ADDR_BLOCK_END  ; self-correcting -- SAFE_ZERO_BYTE is
                                 ; referenced symbolically everywhere it's
                                 ; used (CL-SET/COPY-BUFF/CLEAR-PRB), never
                                 ; by hardcoded address, so this was never
                                 ; really pinned to $3908 -- that literal
                                 ; became stale (and silently truncated
                                 ; ADDR_DONE's own tail by 5 bytes, caught
                                 ; only by investigating an "unaccounted"
                                 ; diff region) the moment ADDR_DISPATCH
                                 ; grew to add the LLIST_TOKEN check.
SAFE_ZERO_BYTE:
        DEFB    $00

CL_PRINTER_FIX:
        BIT     1,(IY+$01)      ; is printer in use?
        JR      Z,CPF_DONE      ; if not (screen case), leave HL untouched
        LD      HL,SAFE_ZERO_BYTE
                                ; printer case: force HL back to the
                                ; single safe byte, discarding whatever
                                ; the column-offset addition computed
CPF_DONE:
        JP      $0ADC           ; PO-STORE, unchanged
CL_PRINTER_FIX_END:

; ---------------------
; RND replacement -- see Decisions.md "Patch 8". All of RND_TABLE,
; RND_INDEX, RND_CARRY live in real RAM (the now-owned PRBUFF, $5B00
; onward -- see the "Ownership note" earlier in Decisions.md), not here;
; this block is the Z80 CODE, in ROM as usual.
;
; NEW_RND_BYTE: one call = one pseudorandom byte in A. Patrik Rak's CMWC
; generator (the same one Boriel BASIC's RND uses -- confirmed via
; Britlion's RandomStream.bas library docs), redesigned to use explicit
; RAM state instead of the original's self-modifying code (which cannot
; work from ROM -- writes to ROM addresses are a no-op). Verified
; byte-for-byte identical to a Python model of the original
; self-modifying-code algorithm across 50,000 draws before writing this
; assembly, and cross-checked again dynamically against 1000 real calls
; of this exact assembled routine with zero mismatches.
;
; RND_BOOT_SEED: one-time table initialization, called once from the
; NEW/cold-boot system-initialization sequence (see the edit near the
; old CLEAR-PRB call further down this file). An uninitialized (RAM-
; garbage) starting table isn't permanently stuck, but produces a long,
; visibly poor-quality warm-up transient (confirmed in Python) before
; recovering -- worth avoiding by seeding properly once at boot rather
; than accepting that on every fresh boot.
;
; RND_RESEED: reseeds the table from a 16-bit value in BC (the
; RANDOMIZE parameter, or FRAMES1 if zero -- same convention the
; original SEED-based RANDOMIZE used). Mixes the seed into each of the
; 8 well-chosen default table entries rather than replacing them
; outright; verified in Python across 2000 random seeds for healthy,
; non-degenerate output (worst case 253/256 distinct bytes in 2000
; draws), and dynamically cross-checked against 5 specific seeds with
; exact byte-for-byte table matches.
;
; NEW_S_RND: replaces the original S-RND. Builds a 16-bit value from two
; NEW_RND_BYTE calls, then reuses the ORIGINAL routine's own STACK-BC +
; exponent-adjustment tail completely unchanged (the same technique the
; original used to turn a raw 16-bit integer into a float in [0,1) by
; subtracting 16 from its exponent byte, i.e. dividing by 65536) --
; jumps to L2630 (S-PI-END) directly rather than the original's L2625,
; since L2625 was nothing but a 2-byte "JR L2630" and no longer exists
; as a separate address once S-RND's original 47-byte block became a
; trampoline.
;
; Sizes verified standalone (pasmo): NEW_RND_BYTE 45 bytes,
; RND_BOOT_SEED 19 bytes, RND_DEFAULT_TABLE 8 bytes, RND_RESEED 41
; bytes, NEW_S_RND 29 bytes -- 142 bytes total.
; ---------------------
        ORG     CL_PRINTER_FIX_END ; self-correcting -- was a hardcoded
                                 ; literal that went stale the moment the
                                 ; ADDR block upstream grew by 5 bytes to
                                 ; add the LLIST_TOKEN check (same fix
                                 ; pattern applied throughout this
                                 ; revision, not hand-recomputed).

RND_TABLE       EQU $5B00       ; 8 bytes, real RAM
RND_INDEX       EQU $5B08       ; 1 byte, real RAM
RND_CARRY       EQU $5B09       ; 1 byte, real RAM
RND_SEED_LO     EQU $5B0A       ; 1 byte, real RAM, scratch during RANDOMIZE
RND_SEED_HI     EQU $5B0B       ; 1 byte, real RAM, scratch during RANDOMIZE
...
; RND_DEFAULT_TABLE has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 9: it now lives inside COS's own spare trampoline padding at
; $37AD (see the 'cos' trampoline at $37AA), reclaiming these 8 bytes
; for the main free block. This comment marks where it used to sit, for
; anyone cross-referencing Checkpoint Alpha's own memory map against
; this build.
...
RND_RESEED_LOOP:
        ...
        INC     HL
        INC     DE
        DJNZ    RND_RESEED_LOOP
        RET
RND_RESEED_END:


; ---------------------
; NEW_S_RND has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 7: it now lives inlined at $25F8, inside its own original
; S-RND trampoline slot, eliminating the JP that used to reach it here
; and reclaiming these 29 bytes for the main free block. This comment
; marks where it used to sit, for anyone cross-referencing Checkpoint
; Alpha's own memory map against this build.
; ---------------------
; RND_BOOT_STUB -- see Decisions.md "Patch 8". Called once, from the
; NEW/cold-boot system-initialization sequence, in place of the
; original direct "CALL L0EDF" (CLEAR-PRB). Preserves that original
; call exactly (CLEAR-PRB is neutered as of Patch 7, so this remains a
; harmless no-op, unchanged from Patch 7's own behavior), then also
; seeds the RND table -- this way RND has healthy output from the very
; first call after boot, not just after a warm-up transient or an
; explicit RANDOMIZE.
; ---------------------
        ORG     RND_RESEED_END  ; self-correcting -- same cascading-drift
                                 ; fix as CL_PRINTER_FIX_END above.

RND_BOOT_STUB:
        CALL    $0EDF           ; CLEAR-PRB (neutered, harmless -- Patch 7)
        CALL    RND_BOOT_SEED
        RET
RND_BOOT_STUB_END:
...
; ---------------------
; THE 'SPARE' LOCATIONS (reduced further -- RND_BOOT_STUB above
; reclaims 7 bytes; 706 remain, see Decisions.md "Changes memory map")
; ---------------------
; NEW_LN_TAIL has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 6: it now lives inlined at $374A, inside its own original
; trampoline slot (see the LN 'GRE.8' section above), eliminating the
; JP that used to reach it here and reclaiming these 36 bytes for the
; main free block. This comment marks where it used to sit, for anyone
; cross-referencing Checkpoint Alpha's own memory map against this
; build.
; ---------------------
; THE 'SPARE' LOCATIONS (reduced further -- NEW_LN_TAIL above reclaims
; 36 bytes; 670 remain, see Decisions.md "Changes memory map")
; ---------------------
; NEW_EXP_CORE has been RELOCATED as of checkpoint_BETA_Decisions.md
; Entry 3: it now lives inlined at $36C4, inside its own original
; trampoline slot (see the 'EXP'/'exp' section above), eliminating the
; JP that used to reach it here and reclaiming these 52 bytes for the
; main free block. This comment marks where it used to sit, for anyone
; cross-referencing Checkpoint Alpha's own memory map against this
; build.
; ---------------------
; THE 'SPARE' LOCATIONS (reduced further -- NEW_EXP_CORE's relocation
; above reclaims a further 52 bytes here; see checkpoint_BETA_Decisions.md
; Entry 3 "Changes memory map")
; ---------------------
; NEW_CIRCLE_CORE / CHECK_AND_PLOT / PLOT4 -- Midpoint Circle Algorithm,
; see Decisions.md. Reached only via the trampoline left at $2331
; (CIRCLE is entered fresh here, the calculator context already closed
; by the preceding end-calc).
;
; Reuses PLOT-SUB ($22E5, unmodified) for actual pixel setting --
; preserving OVER/INVERSE/attribute semantics exactly, verified
; dynamically by drawing the same circle twice with OVER 1 and
; confirming the screen returns to blank -- and FP-TO-BC ($2DA2,
; unmodified) for float-to-signed-16-bit conversion (round-to-nearest).
;
; Per project decision: out-of-bounds points are silently skipped, not
; errored (a deliberate behavior change from the original, which raised
; "Integer out of range" partway through drawing). No BREAK-key
; interruption checks (also deliberately omitted).
;
; REVISED from a first version that unrolled all 8 symmetric points
; into separate inline blocks (422 bytes) -- flagged as excessive, and
; correctly so: refactored into PLOT4, a single shared subroutine that
; plots the 4 sign-combinations of a given (a,b) pair (with the same
; verified zero-avoidance conditions), called twice from the main loop
; -- once for (a,b)=(x,y), once for (a,b)=(y,x), the second call
; skipped entirely when x=y (avoiding the diagonal duplicate). Down to
; 328 bytes, a 94-byte (22%) reduction, reverified against the exact
; same test suite as the original version (7 pixel-perfect circles,
; OVER 1 toggle-twice-to-blank) with identical results -- confirming
; the refactor is behaviorally identical, not just smaller.
;
; The underlying duplicate-avoidance conditions (which of the 4 points
; PLOT4 skips, and when the second PLOT4 call is skipped entirely) were
; verified exhaustively in Python across radii 0-59 before writing this
; assembly: zero duplicates, zero missed points in every case.
;
; CD-PRMS1, DRAW-LINE, and all other original circle/arc infrastructure
; shared with the DRAW command are completely untouched -- confirmed via
; an exhaustive trace of the original ROM's own structure -- since this
; routine never calls into any of it.
;
; Verified standalone (pasmo) then dynamically in the emulator: the same
; 7 test circles (various centers/radii including r=1, r=0.4, and
; centers near screen edges) compared pixel-for-pixel against a Python
; reference implementation, using the REAL ROM's own PIXEL-ADD routine
; as ground truth -- 0 missing pixels, exact total-bit-count match in
; every case. OVER 1 drawn twice returns to a blank screen exactly.
;
; Uses 14 bytes of the PRBUFF-owned scratch RAM ($5B0C-$5B19 -- RND uses
; $5B00-$5B0B; this adds CIRCLE_A/CIRCLE_B, two new scratch slots PLOT4
; needs, on top of the six variables the first version already used).
; ---------------------
        ORG     RND_BOOT_STUB_END ; self-correcting -- same cascading-
                                 ; drift fix as the two anchors above.
PLOT_SUB    EQU $22E5
FP_TO_BC    EQU $2DA2
TEMPS       EQU $0D4D
CIRCLE_CX   EQU $5B0C
...
; --- PLOT4: reads CIRCLE_A, CIRCLE_B (signed 16-bit each). Plots
; (cx+a,cy+b) always; (cx-a,cy+b) if a!=0; (cx+a,cy-b) if b!=0;
; (cx-a,cy-b) if a!=0 and b!=0 -- the exact same conditions verified
; in Python (see Decisions.md) to avoid duplicate-plotting.
PLOT4:
        LD      HL,(CIRCLE_CX)
        LD      DE,(CIRCLE_A)
        ADD     HL,DE
...
; ---------------------
; THE 'SPARE' LOCATIONS (reduced further by every relocation entry in
; checkpoint_BETA_Decisions.md: the CIRCLE routine reclaims 328 bytes;
; NEW_EXP_CORE's relocation (Entry 3) reclaims 52 bytes; NEW_ATN_CORE's
; relocation (Entry 4) reclaims 51 bytes; NEW_COS_CORE's relocation into
; C-ENT's spare padding (Entry 5) reclaims 30 bytes; NEW_LN_TAIL's
; relocation into its own trampoline slot (Entry 6) reclaims 36 bytes;
; NEW_S_RND's relocation into its own trampoline slot (Entry 7) reclaims
; 29 bytes; NEW_TAN_CORE's relocation into CIRCLE's spare padding
; (Entry 8) reclaims 67 bytes; RND_DEFAULT_TABLE's relocation into
; COS's spare padding (Entry 9) reclaims a further 8 bytes -- closing
; out Entry 2's original repacking worklist. 563 bytes were free before
; this revision's ADDR_DISPATCH extension (LLIST switch) shifted this
; whole downstream chain -- now computed from NEW_CIRCLE_CORE_END
; directly, not hand-tracked.)
; ---------------------
        ORG     NEW_CIRCLE_CORE_END ; self-correcting -- was the critical
                                 ; hardcoded "$3ACD" anchor; a stale value
                                 ; here would have caused a silent
                                 ; zero-filled gap or truncated the
                                 ; EXP/to-power/switch/ZX0 code that
                                 ; follows (see Entry 3's own warning
                                 ; about exactly this failure mode, now
                                 ; closed permanently by computing this
                                 ; anchor from real assembled output
                                 ; every time instead of a literal).

; ------------------------------------------------------------
; NEW_EXP_CORE_V2 -- Pade[3/3]: exp(r) ~= N/D,
;   N = 1 + r/2 + r^2/10 + r^3/120
;   D = 1 - r/2 + r^2/10 - r^3/120
; Computed as N=(1+b)+(a+c), D=(1+b)-(a+c) where a=r/2, b=r^2/10,
; c=r^3/120 -- fewer ops than evaluating N and D independently.
; Reduction (round(x/ln2), r, k in mem-3) is reused BYTE-IDENTICAL from
; the shipped, verified Pade[2/2] core -- only the evaluation from "r on
; stack" onward is new. Constants reused verbatim from already-shipped
; patches: 0.2 (from NEW_LN_TAIL), 1/12 (from the old NEW_EXP_CORE) --
; 1/10 and 1/120 are built from these at runtime rather than hand-encoding
; new stk-data bytes, to avoid introducing an unverified float encoding.
; mem-3 = k (untouched throughout, read at the very end). mem-5, 6, 7, 0
; used as scratch -- safe for the same reason the old NEW_EXP_CORE's own
; comment established (EXP is a fresh top-level calculator program;
; nothing else touches these slots during its execution).
; Verified standalone (pasmo): 85 bytes (start.END - start = $8055-$8000).
; ------------------------------------------------------------
NEW_EXP_CORE_V2:
        RST     28H             ;; FP-CALC              x.
        DEFB    $3D             ;;re-stack              x.
        ...
        DEFB    $38,$AA,$3B,$29 ;;(value 1/ln2 -- reused verbatim)
        DEFB    $04             ;;multiply              xln.
        DEFB    $31             ;;duplicate             xln, xln.
        DEFB    $A2             ;;stk-half              xln, xln, 0.5.
        DEFB    $0F             ;;addition              xln, xln+0.5.
        DEFB    $27             ;;int                   xln, k. (round)
        DEFB    $C3             ;;st-mem-3              xln, k.
        DEFB    $03             ;;subtract              r_units.
        DEFB    $34             ;;stk-data
        DEFB    $F0             ;;Exponent: $80, Bytes: 4
        DEFB    $31,$72,$17,$F8 ;;(value ln2 -- reused verbatim)
        DEFB    $04             ;;multiply              r.
        ; --- everything above is byte-identical to the old Pade[2/2]
        ; core through this point. New evaluation below. ---
        DEFB    $31             ;;duplicate             r, r.
        DEFB    $31             ;;duplicate             r, r, r.
        DEFB    $04             ;;multiply              r, r^2.
        DEFB    $C5             ;;st-mem-5              r, r^2.
        DEFB    $02             ;;delete                r.
        DEFB    $E5             ;;get-mem-5             r, r^2.
        DEFB    $34             ;;stk-data
        DEFB    $EE             ;;Exponent: $7E, Bytes: 4
        DEFB    $4C,$CC,$CC,$CD ;;(value 0.2 -- reused from NEW_LN_TAIL)
        DEFB    $A2             ;;stk-half              r, r^2, 0.2, 0.5.
        DEFB    $04             ;;multiply              r, r^2, 0.1.
        DEFB    $04             ;;multiply              r, b.  (b=r^2/10)
        DEFB    $C6             ;;st-mem-6              r, b.
        DEFB    $02             ;;delete                r.
        DEFB    $31             ;;duplicate             r, r.
        DEFB    $A2             ;;stk-half              r, r, 0.5.
        DEFB    $04             ;;multiply              r, a.  (a=r/2)
        DEFB    $01             ;;exchange              a, r.
        DEFB    $E5             ;;get-mem-5             a, r, r^2.
        DEFB    $04             ;;multiply              a, r^3.
        DEFB    $34             ;;stk-data
        DEFB    $EE             ;;Exponent: $7E, Bytes: 4
        DEFB    $4C,$CC,$CC,$CD ;;(0.2 again -- same reused bytes)
        DEFB    $A2             ;;stk-half              a, r^3, 0.2, 0.5.
        DEFB    $04             ;;multiply              a, r^3, 0.1.
        DEFB    $34             ;;stk-data
        DEFB    $ED             ;;Exponent: $7D, Bytes: 4
        DEFB    $2A,$AA,$AA,$AB ;;(value 1/12 -- reused from old EXP core)
        DEFB    $04             ;;multiply              a, r^3, 1/120.
        DEFB    $04             ;;multiply              a, c.  (c=r^3/120)
        DEFB    $0F             ;;addition              (a+c).
        DEFB    $C7             ;;st-mem-7              (a+c).
        DEFB    $02             ;;delete                --.
        DEFB    $E6             ;;get-mem-6             b.
        DEFB    $A1             ;;stk-one               b, 1.
        DEFB    $0F             ;;addition              1+b.
        DEFB    $C6             ;;st-mem-6              1+b.  (overwrite)
        DEFB    $02             ;;delete                --.
        DEFB    $E6             ;;get-mem-6             1+b.
        DEFB    $E7             ;;get-mem-7             1+b, (a+c).
        DEFB    $0F             ;;addition              N.
        DEFB    $C0             ;;st-mem-0              N.
        DEFB    $02             ;;delete                --.
        DEFB    $E6             ;;get-mem-6             1+b.
        DEFB    $E7             ;;get-mem-7             1+b, (a+c).
        DEFB    $03             ;;subtract              D.
        DEFB    $E0             ;;get-mem-0             D, N.
        DEFB    $01             ;;exchange              N, D.
        DEFB    $05             ;;division              pade_result.
        DEFB    $E3             ;;get-mem-3             pade_result, k.
        DEFB    $38             ;;end-calc
        JP      $36F9           ; reused original tail, unchanged

; ------------------------------------------------------------
; TO_POWER_V2 -- new to-power head, integer-exponent fast path via
; exponentiation-by-squaring. Reached only via the trampoline left at
; $3851. Preserves 0^anything semantics through a LOCAL copy (XIS0_LOCAL)
; rather than jumping back to the original XIS0/ONE/LAST, since a
; calculator jump-true can't cross the end-calc this design needs, nor
; reach code relocated this far away with a Z80 JP into calculator-opcode
; bytes.
; Standalone-assembled at 164 bytes originally; the boolean-test bugfix
; below (jump-true replacing the broken Z80 byte-peek for isWhole/isZero/
; isPos) shrank the real in-context size to 140 bytes ($3B22-$3BAD,
; verified against the actual assembled binary).
; ------------------------------------------------------------
TO_POWER_SIGN   EQU     $5B1E   ; 1 free RAM byte in PRBUFF -- NOT $5B1A,
                                 ; which is reserved for the LLIST switch's
                                 ; 4-byte parameter slice ($5B1A-$5B1D).
                                 ; 0=Y positive, 1=Y negative, consulted
                                 ; only by SQ_LOOP_ENTRY's final reciprocal
                                 ; step.
TO_POWER_V2:
        RST     28H             ;; FP-CALC              X, Y.
        DEFB    $01             ;;exchange              Y, X.
        DEFB    $3D             ;;re-stack              Y, X.  (normalize
                                 ;; short-int to full float so its exponent
                                 ;; byte is meaningful)
        DEFB    $38             ;;end-calc              Y, X.
        LD      A,(HL)          ; X's exponent byte -- $00 means X is zero
                                 ; -- legitimate: X was just re-stacked to
                                 ; full-float, so byte0==0 unambiguously
                                 ; means "value is 0.0" here.
        AND     A
        JP      Z,XIS0_LOCAL

; X != 0. Test whether Y is an exact whole number, staying INSIDE this
; same open calculator context so jump-true can correctly interpret the
; isWhole boolean (short-int format -- a Z80 byte-peek after end-calc
; cannot distinguish true from false here, since short-int's own byte 0
; is always $00 regardless of value; this is what jump-true exists for).
        RST     28H             ;; FP-CALC              Y, X.
        DEFB    $01             ;;exchange              X, Y.
        DEFB    $31             ;;duplicate             X, Y, Y.
        DEFB    $31             ;;duplicate             X, Y, Y, Y.
        DEFB    $3A             ;;truncate              X, Y, Y, trunc(Y).
        DEFB    $03             ;;subtract              X, Y, frac.
        DEFB    $30             ;;not                   X, Y, isWhole.
        DEFB    $00             ;;jump-true
        DEFB    TP_WHOLE-$      ;;to TP_WHOLE (still-open context; jump-true
                                 ;; consumes isWhole correctly either way)

; not whole -- jump-true consumed isWhole, stack is [X, Y]. Fall straight
; through to the ORIGINAL calculator sequence, byte-identical to what
; to-power always did.
        DEFB    $01             ;;exchange              Y, X.
        DEFB    $25             ;;ln                    Y, LN X.
        DEFB    $04             ;;multiply              Y*LN X.
        DEFB    $38             ;;end-calc
        JP      L36C4           ; EXP's entry (this patch's own trampoline)

TP_WHOLE:
; Y confirmed an exact integer, X != 0. Stack: [X, Y] (jump-true already
; consumed isWhole -- same convention the original ONE/LAST labels use).
        DEFB    $31             ;;duplicate             X, Y, Y.
        DEFB    $38             ;;end-calc              X, Y, Y.
        CALL    FP_TO_BC        ; $2DA2, unmodified -- consumes the top
                                 ; Y-copy. REAL contract (confirmed from
                                 ; FP-TO-BC's own source, not assumed):
                                 ; BC = |Y| (unsigned magnitude), carry
                                 ; flag = set if too big to represent,
                                 ; zero flag = set if Y was positive,
                                 ; reset (NZ) if Y was negative. Check
                                 ; flags immediately -- nothing between
                                 ; CALL and here can have touched them.
        JP      C,FALLBACK_DROP0   ; too big for FP-TO-BC itself -> fallback
        JR      Z,TP_SIGN_POS
        LD      A,1
        LD      (TO_POWER_SIGN),A
        JR      TP_MAG_CHECK
TP_SIGN_POS:
        ...
        LD      (TO_POWER_SIGN),A
TP_MAG_CHECK:
; BC now holds |Y| unsigned. Need |Y| <= 127 for our declared range.
        LD      A,B
        AND     A
        JP      NZ,FALLBACK_DROP0  ; B!=0 -> magnitude >= 256, too big
        LD      A,C
        CP      128
        JP      NC,FALLBACK_DROP0  ; magnitude >= 128 -> too big
RANGE_OK:
; C = |Y| (0-127), TO_POWER_SIGN set. Stack still holds [X, Y] (the
; untouched original Y, since FP_TO_BC consumed only its duplicate).
        PUSH    BC              ; preserve |Y| -- the calculator dispatcher
                                 ; clobbers BC internally as scratch (found
                                 ; via emulator register tracing: a bare
                                 ; RST 28H/delete/end-calc changed BC from
                                 ; $0002 to $005E with no explicit BC-using
                                 ; literal anywhere in the program).
        RST     28H             ;; FP-CALC              X, Y.
        DEFB    $02             ;;delete                X.
        DEFB    $38             ;;end-calc              X.
        POP     BC              ; restore |Y|
        CALL    SQ_LOOP_ENTRY   ; leaves [result] on the stack
        RET

FALLBACK_DROP0:
; stack is exactly [X, Y] here (FP_TO_BC consumed only the extra
; duplicate); the calculator context was closed by TP_WHOLE's own
; end-calc before FP_TO_BC ran, so this needs its own fresh RST 28H.
        RST     28H             ;; FP-CALC              X, Y.
        DEFB    $01             ;;exchange              Y, X.
        DEFB    $25             ;;ln                    Y, LN X.
        DEFB    $04             ;;multiply              Y*LN X.
...
; ------------------------------------------------------------
; XIS0_LOCAL -- local copy of the original XIS0/ONE/LAST semantics
; (0^0=1, 0^+n=0, 0^-n=overflow via 1/0). The ORIGINAL bytes at
; $385D-$386D are untouched and still independently reachable. Structured
; as ONE continuous calculator program (single RST 28H to single
; end-calc), using jump-true throughout to correctly interpret the
; isZero/isPos calculator booleans -- exactly mirroring the original
; XIS0/ONE/LAST code's own proven structure, not the flawed Z80-byte-peek
; pattern this patch's TO_POWER_V2 fixed above.
; ------------------------------------------------------------
XIS0_LOCAL:
        RST     28H             ;; FP-CALC              Y, X(=0).
        DEFB    $02             ;;delete                Y.
        DEFB    $31             ;;duplicate             Y, Y.
        DEFB    $30             ;;not                   Y, isZero.
        DEFB    $00             ;;jump-true
        DEFB    XL_ONE-$        ;;to XL_ONE             if Y is zero.

        DEFB    $A0             ;;stk-zero              Y, 0.
        DEFB    $01             ;;exchange              0, Y.
        DEFB    $37             ;;greater-0             0, isPos.
        DEFB    $00             ;;jump-true
        DEFB    XL_LAST-$       ;;to XL_LAST            if Y was positive.

        DEFB    $A1             ;;stk-one               0, 1.
        DEFB    $01             ;;exchange              1, 0.
        DEFB    $05             ;;division              1/0  (overflow --
                                 ;; Y was negative)
XL_ONE:
        DEFB    $02             ;;delete                --.
        DEFB    $A1             ;;stk-one               1.
XL_LAST:
        DEFB    $38             ;;end-calc              result is 1 or 0
                                 ;; (or overflow already raised above).
        RET

; ------------------------------------------------------------
; SQ_LOOP_ENTRY -- exponentiation by squaring. Entry: X on the FP
; calculator stack, C = |Y| (0-127) in the Z80 C register, TO_POWER_SIGN
; set. Exit: [X^Y] on the FP calculator stack.
; ------------------------------------------------------------
SQ_LOOP_ENTRY:
        PUSH    BC              ; preserve |Y| across this setup call too --
                                 ; same BC-clobbering behavior as RANGE_OK.
        RST     28H             ;; FP-CALC              X.
        DEFB    $C1             ;;st-mem-1              X.  (mem1=base)
        DEFB    $02             ;;delete                --.
        DEFB    $A1             ;;stk-one               1.  (result=1)
        DEFB    $38             ;;end-calc              result.
        POP     BC
SQ_LOOP:
        LD      A,C
        ...
        BIT     0,C
        JR      Z,SQ_SKIP_MUL
        PUSH    BC              ; preserve C (remaining bit count) across
                                 ; the calculator call below
        RST     28H             ;; FP-CALC              result.
        DEFB    $E1             ;;get-mem-1             result, base.
        DEFB    $04             ;;multiply              result*base.
        DEFB    $38             ;;end-calc              new result.
        POP     BC
SQ_SKIP_MUL:
        PUSH    BC              ; preserve C across this calculator call too
        RST     28H             ;; FP-CALC              result.
        DEFB    $E1             ;;get-mem-1             result, base.
        DEFB    $31             ;;duplicate             result, base, base.
        DEFB    $04             ;;multiply              result, base^2.
        DEFB    $C1             ;;st-mem-1              result, base^2.
        DEFB    $02             ;;delete                result.
        DEFB    $38             ;;end-calc              result.
        POP     BC
        SRL     C
        JR      SQ_LOOP
SQ_DONE:
        ...
        RET
TO_POWER_BLOCK_END:
; Marker label, not a real routine -- gives the free-space ORG anchor
; below something to be COMPUTED FROM rather than hand-estimated. This
; directly closes the failure mode just caught during verification: a
; hand-typed "= $3ACD+85+140" comment went stale the moment SQ_LOOP_ENTRY
; grew by 8 bytes (BC-preservation fix), and the resulting backward ORG
; silently truncated SQ_DONE's own tail -- exactly the class of bug
; Safety.md's own worked example warns about. This label removes the
; hand-computation step entirely for all future edits to this block.

; ---------------------
; ---------------------
; THE 'SPARE' LOCATIONS -- remaining free space after NEW_EXP_CORE_V2
; and the to-power fast-path block. Byte count is NOT hand-computed --
; the ORG anchor below is symbolic (TO_POWER_BLOCK_END), and the exact
; fill count is computed programmatically from the real assembled
; binary (see the build process notes in Decisions.md). This closes
; the failure mode caught during this patch's own verification: a
; hand-typed '= $3ACD+85+140'-style comment went stale the moment the
; block above grew by 8 bytes (BC-preservation fix), silently
; truncating real code -- exactly Safety.md's own backward-ORG warning,
; caught only because every edit here is verified against the actual
; assembled bytes, never trusted from a clean exit code alone.
        ORG     TO_POWER_BLOCK_END ; self-correcting -- always exactly
                                 ; where the block above really ends.

; ------------------------------------------------------------
; S_LLIST_SWITCH -- the LLIST(expr) mid-expression dispatch switch.
; Reached via ADDR_DISPATCH's own extended hook (see the edit near
; S-ALPHNUM above) -- same mechanism as ADDR, checking a second
; reclaimed token. LLIST's own statement form (typed at line-start) is
; completely untouched, same principle as LPRINT/ADDR.
;
; Governing decisions (see Decisions.md): NOT interrupt-safe by design --
; assumes BASIC-only entry, which holds because BASIC's interpreter never
; runs from inside the ISR on this ROM. The parameter slice (LLIST_SLICE,
; 4 bytes at $5B1A-$5B1D) carries NO persistence guarantee before, during,
; or after a call -- each target routine treats it as private scratch.
; Bad/out-of-range slice DATA is the calling program's own responsibility
; to get right (same "tough luck" contract as USR's own address argument)
; -- but the SELECTOR itself (this routine's own expr argument) is
; sanitized here, since it's structural control flow, not payload data.
;
; Syntax: LLIST(expr) -- expr is a general numeric expression (not just a
; variable, unlike ADDR), so this must manually replicate what SCANNING
; does for a real numeric-function token: consume '(', recursively call
; SCANNING to evaluate expr, consume ')'.
; ------------------------------------------------------------
LLIST_SLICE     EQU     $5B1A   ; 4-byte parameter slice, reserved
                                 ; ($5B1A-$5B1D). Not $5B1E onward --
                                 ; that's TO_POWER_SIGN, defined earlier.
SCANNING        EQU     $24FB
LLIST_TABLE_COUNT EQU   2       ; number of registered switch targets --
                                 ; update when adding more (see LLIST_TABLE)

S_LLIST_SWITCH:
        RST     20H             ; consume the LLIST token
        CP      $28             ; expect '('
        JP      NZ,REPORT_C
        RST     20H             ; consume '(' -> CH_ADD at the expression

        CALL    SCANNING        ; evaluates expr, result on FP calc stack,
                                 ; advances CH_ADD past it (also correctly
                                 ; handles syntax-check mode internally,
                                 ; same as any real numeric function would)
        RST     18H             ; GET-CHAR
        CP      $29             ; expect ')'
        JP      NZ,REPORT_C
        RST     20H             ; consume ')'
        ...
        CALL    SYNTAX_Z
        JR      Z,SLS_SYNTAX    ; syntax-check pass -- don't dispatch for
                                 ; real, matching S_ADDR's own SA_SYNTAX
                                 ; precedent

        CALL    L2DD5           ; FP-TO-A: A=selector magnitude, carry set
                                 ; if magnitude doesn't fit 0-255, zero
                                 ; flag set if the value was positive
                                 ; (confirmed from FP-TO-A/FP-TO-BC's own
                                 ; disassembly, not assumed)
        JP      C,REPORT_C       ; out of 0-255 range -> Nonsense in BASIC
        JR      NZ,SLS_BADSEL    ; negative selector -> also invalid

        CP      LLIST_TABLE_COUNT
        JP      NC,SLS_BADSEL    ; no routine registered at this index

        LD      HL,LLIST_TABLE
        LD      D,0
        LD      E,A
        ADD     HL,DE
        ADD     HL,DE            ; HL = LLIST_TABLE + selector*2
        LD      E,(HL)
        INC     HL
        LD      D,(HL)           ; DE = target routine's address

        LD      HL,SLS_RETURN
        PUSH    HL               ; fake return address, USR-style
        EX      DE,HL
        JP      (HL)             ; "CALL (HL)" via push+jp

SLS_RETURN:
; Every switch target's own contract: reached with no defined register
; state beyond being invoked (same as USR); on exit, leaves its numeric
; result in BC and RET.
        CALL    STACK_BC        ; stacks BC as the numeric result
        JP      S_NUMERIC       ; rejoin the ROM's own shared continuation

SLS_SYNTAX:
        LD      BC,$0000        ; safe placeholder during syntax-check
        CALL    STACK_BC
        JP      S_NUMERIC
...
; ------------------------------------------------------------
; LLIST_TABLE -- the switch's dispatch table. One 2-byte address per
; registered selector value. Update LLIST_TABLE_COUNT above when adding.
; ------------------------------------------------------------
LLIST_TABLE:
        DEFW    ZX0_DECOMPRESS  ; LLIST(0) -- ZX0 decompression, see below
        DEFW    CACHE_INSERT    ; LLIST(1) -- GOTO/GOSUB/NEXT line-address
                                 ; cache insertion, see below

; ------------------------------------------------------------
; ZX0_DECOMPRESS -- LLIST(0) target. Reads slice[0..1]=source address
; (16-bit, little-endian), slice[2..3]=destination address (16-bit,
; little-endian), then calls the unmodified ZX0 "Standard" decompressor
; (DZX0_STANDARD, 68 bytes, by Einar Saukas & Urusergi -- see the
; project's attribution notes; confirmed ROM-safe: no self-modifying
; code, verified directly from its own source, unlike "Turbo"/"Fast"/
; "Mega" which all rely on writing into their own instruction stream).
;
; Same "tough luck" contract as everything address-driven in this
; project: the slice's source/dest addresses are the calling program's
; own responsibility to get right. A bad source address reads garbage
; compressed data (undefined decompression result, not a crash by
; itself); a bad destination address writes decompressed bytes wherever
; it points, which can absolutely crash or corrupt the system -- exactly
; the same risk profile a bad USR address already carries.
;
; Returns 0 in BC always -- ZX0's own decompressor doesn't report a
; length or error condition (by design, per its own documentation: fully
; trusts the compressed stream's own end marker), so there's nothing
; meaningful to return beyond "the call completed."
; ------------------------------------------------------------
ZX0_DECOMPRESS:
        LD      HL,(LLIST_SLICE)       ; source address (slice bytes 0-1)
        LD      DE,(LLIST_SLICE+2)     ; dest address (slice bytes 2-3)
        CALL    DZX0_STANDARD
        LD      BC,0
        RET

; ------------------------------------------------------------
; DZX0_STANDARD -- ZX0 "Standard" decoder, unmodified, by Einar Saukas &
; Urusergi (https://github.com/einar-saukas/ZX0). 68 bytes. Parameters:
; HL=source (compressed data), DE=destination (decompressing). Uses only
; AF, BC, DE, HL, and the Z80 hardware stack -- confirmed no self-
; modifying code anywhere in this routine (verified directly against the
; upstream source before use, unlike Turbo/Fast/Mega which all rely on
; writing into their own instruction stream and are NOT ROM-safe as a
; result -- see the project's own research notes on this).
; ------------------------------------------------------------
DZX0_STANDARD:
...
DZX0S_NEW_OFFSET:
        ...
        INC     HL
        RR      B                        ; last offset bit becomes first
                                          ; length bit
        RR      C
...
; ------------------------------------------------------------
; GOTO/GOSUB/RETURN/NEXT line-address cache -- LLIST(1) inserts,
; a single interception at LINE-NEW (see the edit near LINE-NEW's own
; "CALL LINE_ADDR" below) transparently accelerates lookups.
;
; All four of GOTO, GOSUB, RETURN, and NEXT funnel through one single
; choke point: LINE-NEW, which makes exactly one call to LINE-ADDR
; (confirmed by tracing GO-TO/GO-SUB/RETURN/NEXT's own source -- GO-SUB
; and RETURN both jump into GO-TO-2, and NEXT's loop-continuation does
; too -- none of them call LINE-ADDR directly). LINE-ADDR itself does a
; linear scan from PROG, O(n) in how far into the program the target
; line sits -- this is the actual cost being cached.
;
; Confirmed by tracing register flow through LINE-USE/NEXT-LINE (the
; code that runs immediately after an exact match): DE, as returned by
; LINE-ADDR, is completely dead by the time it would matter -- LINE-USE
; recomputes its own DE from scratch by reading the found line's own
; record. A cache hit therefore only needs to produce HL (the address)
; and the Z flag (exact match) -- nothing else LINE-ADDR returns is
; ever consumed downstream of a match. Statement-within-line offsetting
; (for NEXT's non-zero-statement case) happens entirely within
; EACH-STMT, working only from HL, bounded by statements-per-line, not
; by program size -- orthogonal to this cache, no special handling
; needed.
;
; Storage: 32 slots x 4 bytes (line_hi, line_lo, addr_hi, addr_lo) in
; the tail 128 bytes of PRBUFF ($5B80-$5BFF). Empty slot = line number
; field is $0000 (line 0 is never a valid BASIC line number). Insertion
; scratch is carved from the free space immediately below the cache
; block, not from registers, to avoid the exact class of
; register-preservation bug this project's own to-power patch hit
; twice -- everything needed after CALL LINE_ADDR is either re-read from
; scratch or, where safe, carried in a register CACHE_SLOT_ADDR itself
; never touches (BC).
;
; Governing decisions (see Decisions.md): no automatic invalidation on
; program edits (add/delete/renumber) -- entirely the caller's
; responsibility, "tough luck" if a stale slot is consulted after an
; edit that moved the cached line, same tier as this project's other
; address-trusting mechanisms. Slot range (0-31) and line existence ARE
; validated at insertion time (unlike the address-trusting cases
; elsewhere) because an unchecked slot number is a raw array index into
; ROM-adjacent memory, not just caller-owned data -- a genuine
; structural safety boundary, not a data-sanity nicety.
; ------------------------------------------------------------
CI_SLOT         EQU     $5B78
CI_LINEHI       EQU     $5B79
CI_LINELO       EQU     $5B7A
CACHE_TARGET_HI EQU     $5B7D
CACHE_TARGET_LO EQU     $5B7E
CACHE_SLOT_IDX  EQU     $5B7F
CACHE_BASE      EQU     $5B80   ; 128 bytes, 32 slots of 4
CACHE_SLOTS     EQU     32
LINE_ADDR       EQU     $196E   ; real ROM LINE-ADDR, unmodified

; ------------------------------------------------------------
; CACHE_INSERT -- LLIST(1) target. Reads slice[0]=slot (0-31),
; slice[1..2]=line number (little-endian: lo,hi, matching this
; project's established slice convention). slot=0,line=0 clears the
; whole cache. Otherwise validates slot range and line existence (via
; the real, unmodified LINE-ADDR) before writing anything -- an
; invalid slot or nonexistent line leaves the cache completely
; untouched. Returns the cached line's address in BC on success, 0 on
; any failure (bad slot, line not found, or a plain clear-all).
; ------------------------------------------------------------
CACHE_INSERT:
        LD      A,(LLIST_SLICE)
        LD      (CI_SLOT),A
        LD      B,A                     ; running OR-accumulator for the
                                         ; all-zero (clear-all) test below
        LD      A,(LLIST_SLICE+2)      ; line_hi
        LD      (CI_LINEHI),A
        OR      B
        LD      B,A
        LD      A,(LLIST_SLICE+1)      ; line_lo
        LD      (CI_LINELO),A
        OR      B                       ; A = slot OR line_hi OR line_lo
        JR      NZ,CI_NORMAL            ; any nonzero -> not a clear request
        CALL    CACHE_CLEAR
        LD      BC,0
        RET
...
CI_NORMAL:
        LD      A,(CI_SLOT)
        CP      CACHE_SLOTS
        JR      NC,CI_FAIL              ; slot out of 0-31 range

        LD      A,(CI_LINEHI)
        LD      H,A
        LD      A,(CI_LINELO)
        LD      L,A
        CALL    LINE_ADDR               ; real LINE-ADDR: HL=addr, Z=exact match
        JR      NZ,CI_FAIL              ; line does not exist -- refuse insert

        LD      B,H                     ; stash address in BC (CACHE_SLOT_ADDR
        LD      C,L                     ; doesn't touch B/C) -- this also
                                         ; happens to already be our return
                                         ; value, so no extra work needed later
        LD      A,(CI_SLOT)
        CALL    CACHE_SLOT_ADDR         ; A=slot -> HL=this slot's address
...
; ------------------------------------------------------------
; CACHE_SLOT_ADDR -- shared helper. Entry: A = slot index (0-31).
; Exit: HL = that slot's address (CACHE_BASE + slot*4).
; ------------------------------------------------------------
CACHE_SLOT_ADDR:
        LD      L,A
        LD      H,0
...
; ------------------------------------------------------------
; CACHE_CLEAR -- zeroes every slot's line-number field (marking all 32
; slots empty). Address fields are left untouched -- they're only ever
; read when the line-number field is nonzero, so their stale content
; is harmless.
; ------------------------------------------------------------
CACHE_CLEAR:
        LD      HL,CACHE_BASE
...
CACHE_CLEAR_LOOP:
        ...
        INC     HL
        INC     HL
        DJNZ    CACHE_CLEAR_LOOP
        RET

; ------------------------------------------------------------
; CACHE_LOOKUP_OR_REAL -- transparent drop-in replacement for a direct
; "CALL LINE-ADDR". Entry: HL = target line number (same contract as
; LINE-ADDR itself). Exit: HL = address, Z flag = exact match -- either
; from a cache hit, or (falling through unchanged) from the real
; LINE-ADDR itself on a miss. Nothing downstream of the original call
; site needs to know which happened.
; ------------------------------------------------------------
CACHE_LOOKUP_OR_REAL:
        LD      A,H
        OR      L
        JP      Z,LINE_ADDR     ; target line 0 is never a valid cached
                                 ; entry (it's also the empty-slot
                                 ; sentinel) -- always defer to the real
                                 ; search rather than risk a false hit
                                 ; against an empty slot's stale address
                                 ; bytes. HL is already the target (0),
                                 ; which is exactly LINE_ADDR's own
                                 ; entry contract.
        LD      A,H
        LD      (CACHE_TARGET_HI),A
        LD      A,L
        LD      (CACHE_TARGET_LO),A
        XOR     A
        LD      (CACHE_SLOT_IDX),A
CACHE_SCAN_LOOP:
        LD      A,(CACHE_SLOT_IDX)
        ...
        CALL    CACHE_SLOT_ADDR         ; A=slot idx -> HL=slot address

        LD      A,(HL)                  ; slot's line_hi
        LD      D,A
        LD      A,(CACHE_TARGET_HI)
        CP      D
        JR      NZ,CACHE_SCAN_ADVANCE

        INC     HL
        LD      A,(HL)                  ; slot's line_lo
        LD      D,A
        LD      A,(CACHE_TARGET_LO)
        CP      D
        JR      NZ,CACHE_SCAN_ADVANCE

        ; MATCH -- HL currently points at slot+1 (line_lo). Read the
        ; address from slot+2/slot+3 into H/L via A as intermediary
        ; (can't use HL as both pointer and destination at once).
        INC     HL
        LD      A,(HL)                  ; addr_hi
        LD      D,A
        INC     HL
        ...
        LD      E,A
        LD      H,D
        LD      L,E                     ; HL = cached address
        CP      A                       ; force Z flag set (A compared to
                                         ; itself is always equal) -- signals
                                         ; "exact match" to the caller
        RET
...
CACHE_MISS:
        ...
        LD      A,(CACHE_TARGET_LO)
        LD      L,A
        JP      LINE_ADDR               ; tail-call the real LINE-ADDR --
                                         ; its own RET returns straight to
                                         ; OUR caller, fully transparent

LLIST_SWITCH_BLOCK_END:
; Marker label, same self-correcting-ORG discipline as TO_POWER_BLOCK_END.
        ORG     LLIST_SWITCH_BLOCK_END
...
; PASMO PORT: TASM's #end directive removed -- pasmo has no such directive,
; end-of-file is implicitly end-of-assembly.
